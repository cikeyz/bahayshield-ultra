...
Final controller: **Arduino Mega 2560 Pro** (ATmega2560, 5 V logic).
...
Source of truth: `firmware/bahayshield-ultra/pins.h`.
...
| PCF8574 keypad expander | `0x20` | Key map handled in firmware |
| DS3231 RTC | `0x68` | |
...
## Relay module header order (HL-52S class)

Logic header: `GND IN1 IN2 VCC`.

Contact group per channel: `NO COM NC`.

- Relay 1: `COM` = load supply in, `NC` = load out (opens when the coil energizes).
